    ...
    └── MultiIndexer.Run() : errgroup, one goroutine per chain
        └── ChainIndexer.run()
            └── poll loop (every POLLING_INTERVAL_MS ms)
                └── TaskCreated first, then 10 more handlers in parallel
...
| `DeadlineExtended`    | `UpdateTaskDeadline`: store the new deadline                            |
| `Withdrawn`           | `RecordWithdrawal`: insert withdrawal row (idempotent via unique index) |
